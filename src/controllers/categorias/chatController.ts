...
export class ChatController {
    ...
    /**
     * Transforma tempo (em millisegundos) de UTC para local
     * @param time tempo (em millisegundos)
     * @returns tempo transformado
     */
    static getLocalFromUTCTime(time: number): number {
        return time - 60000 * new Date().getTimezoneOffset();
    }


    FindMessagesFromDB(req: TypedRequestBody<any>, res: Response, next: any) {
        var numero: string = req.body.numero;
        var timestampMaisRecente: string = req.body.timestampMaisRecente;
        console.log(timestampMaisRecente);


    if (timestampMaisRecente != undefined) {
        // console.log(new Date(ChatController.getLocalFromUTCTime(new Date(timestampMaisRecente).getTime())))
        const timestampDate = new Date(timestampMaisRecente);
        console.log(timestampDate);

        ChatController.repository.chat.find({
            select: ['remoteJid', 'fromMe', 'id', 'pushName', 'text', 'messageType', 'messageTimestamp', 'thumb', 'instance', 'chatWith'],
            order: { messageTimestamp: "DESC" },
            where: { chatWith: (numero), messageTimestamp: MoreThan(timestampDate)},
        }).then(u => {
...
chatRouter.post('/', controller.FindMessagesFromDB);
